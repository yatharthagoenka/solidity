...
)
{
	...
	error["severity"] = Error::formatErrorCategoryLowercase(_severity);
...
Json::Value formatFatalError(string const& _type, string const& _message)
{
	...
	output["errors"].append(formatError(Error::Category::Error, _type, "general", _message));
	return output;
}
	...
	for (auto const& sourceName: sources.getMemberNames())
	{
		...
		else if (sources[sourceName]["urls"].isArray())
		{
			...
			for (auto const& failure: failures)
			{
				...
				ret.errors.append(formatError(
					found ? Error::Category::Warning : Error::Category::Error,
					"IOError",
					"general",
					failure
				));
...
Json::Value StandardCompiler::compileSolidity(StandardCompiler::InputsAndSettings _inputsAndSettings)
{
	...
	catch (Error const& _error)
	{
		errors.append(formatErrorWithException(
			compilerStack,
			_error,
			Error::errorCategory(_error.type()),
			_error.typeName(),
			"general",
			"Uncaught error: "
		));
...
Json::Value StandardCompiler::compileYul(InputsAndSettings _inputsAndSettings)
{
	...
	output["errors"] = Json::arrayValue;
	output["errors"].append(formatError(Error::Category::Warning, "Warning", "general", "Yul is still experimental. Please use the output with care."));
